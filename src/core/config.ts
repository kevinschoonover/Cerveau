...
> = [
  [
    ["--port-offset"],
    {
      ...
      defaultValue: 0,
      type: "int",
      help: "port offset for the default port values"
    }
  ],
  ...
  [
    ["--tcp-port"],
    {
      ...
      defaultValue: 3000,
      type: "int",
      help: "the port that TCP clients should connect through"
    }
  ],
  ...
  [
    ["--http-port"],
    {
      ...
      defaultValue: 3080,
      type: "int",
      help: "the port that the web interface should be accessed through"
    }
  ],
  ...
  [
    ["--ws-port"],
    {
      ...
      defaultValue: 3088,
      type: "int",
      help: "the port that WebSocket clients should connect though"
    }
  ],
  ...
  [
    ["--single-threaded"],
    {
      ...
      help:
        "If game sessions should be ran on the master thread, for easier debugging of game logic"
    }
  ],
  ...
  [
    ...
    {
      action: "storeFalse",
      dest: "GAME_SETTINGS_ENABLED",
      help: "ignores any requested game settings from clients",
      defaultValue: true
    }
  ],
  ...
  [
    ...
    {
      action: "storeFalse",
      dest: "LOAD_EXISTING_GAMELOGS",
      help:
        "gamelogs are not loaded to get the status or view in the web interface",
      defaultValue: true
    }
  ],
  ...
  [
    ...
    {
      action: "storeFalse",
      dest: "SESSION_TIMEOUTS_ENABLED",
      help: "disables sessions timing out, most useful for debugging",
      defaultValue: true
    }
  ],
  ...
  [
    ["--no-api"],
    {
      action: "storeFalse",
      dest: "API_ENABLED",
      help: "does not run the hooks for the REST API service",
      defaultValue: true
    }
  ],
  ...
  [
    ["--no-web"],
    {
      action: "storeFalse",
      dest: "WEB_ENABLED",
      help: "does not run the web interface",
      defaultValue: true
    }
  ],
  ...
  [
    ...
    {
      action: "storeFalse",
      dest: "UPDATER_ENABLED",
      help: "does not run the update checker",
      defaultValue: UPDATER_ENABLED_BY_DEFAULT
    }
  ],
...
for (const key of Object.keys(parsedArgs)) {
  ...
  // default/cli value
  let value =
    commandLineValue === defaults[key] && envValue
  ...
  if (arg && arg.nargs && !Array.isArray(value)) {
    // It is not an array, but should be (probably string from dotenv)
    value =
      value === undefined
        ? []
        : String(value)
